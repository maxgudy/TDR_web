...
const pages = {
    ...
    'faq-email': {
        ...
        subtitle: 'Max Gudayol · Automatitzacions amb IA',
        webhookUrl: 'https://m9m.fluxaroo.com/webhook/f0c0029a-ede7-435e-be1b-0597790d6aa1/chat'
    },
    'dades-estudiants': {
        title: 'Agent Directius',
        subtitle: 'Max Gudayol · Automatitzacions amb IA',
        webhookUrl: 'https://m9m.fluxaroo.com/webhook/53d6308a-1a28-4da4-a335-6c3f14c3f6d4/chat'
    }
};
...
function initPageSpecificFeatures(pageId) {
    ...
    if (pageId === 'dades-estudiants' && typeof initDashboard === 'function') {
        setTimeout(() => {
            if (window.supabaseClient) {
                initDashboard();
            } else {
                console.warn('[Main] Supabase client no disponible per al dashboard, reintentant...');
                setTimeout(() => {
                    if (window.supabaseClient && typeof initDashboard === 'function') {
                        initDashboard();
                    }
                }, 500);
            }
